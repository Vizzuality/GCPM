class AddUserSearchesView < ActiveRecord::Migration[5.0]
  def up
    execute <<-SQL
      DROP VIEW searches;
      ...
      CREATE INDEX index_users_on_position        ON users USING gin(to_tsvector('english', position));
    SQL
  end
  ...
  def down
    execute <<-SQL
      ...
      DROP INDEX index_users_on_position;
    SQL
  end
end
